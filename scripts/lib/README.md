...
| `util.mjs` | `makeLoc` / `toNum` / `gmIds` / `isPrimaryGM` / `libStorage` / `announceChange` / `ownsSheet` / `typeScale` + `atTypeScale` — the helpers every feature used to copy, the one predicate that tells a sheet this module draws from the system's, and the JS twin of `--acks-extras-k` for a window whose opening size is a design-canvas figure. |
