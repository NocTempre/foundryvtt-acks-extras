...
| An NPC statline holds another block's words: a neighbour's spells among its proficiencies, a table's number inside a hyphenated word | Detection sets the statline page's next column past where that column's text begins, so the block's bound admits the neighbour's runs and the box around them reads both | `assists.statColumns` per entry (not `flowColumns`, which turns the prose flow onto that page) — DECISIONS.md 2026-09-23 |
| A proficiency token's specialty has no opening bracket | The book's own text layer omits the character; no box can restore it | `rebracket` when `resolveAbilities` reads the token — DECISIONS.md 2026-09-23 |
