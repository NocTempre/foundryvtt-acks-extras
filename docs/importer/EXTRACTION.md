...
| An OSE page detects as ONE column: a heading on the left ends a passage on the right, and a stat-block header row lands where a description belongs | `detectColumns` votes on where body runs start, and a lopsided page carries a single column home | NOT fixed — a per-page column override was built and rejected: declaring the second column lets every entry walk into a column that CONTINUES an earlier page, so they swallow each other. Cases ledgered in `tools/importer/prose-stops.json`; DECISIONS.md |
