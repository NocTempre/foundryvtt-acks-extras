...
structure, pointers, and extraction assists. See `RECIPES.md` for the
pipeline that produces it and `BINDING-FOUNDRY.md` for the first consumer.
...
| `text` | `paras: [ { box, section?, dropText?, fixes?, continues? } … ]` | `[{section?, text}]` | assemble prose: for each para box, take items in reading order (y, then x), join, apply `fixes`/`dropText`. Paragraph boundaries AND section labels (`combat`, `ecology`, …) were computed offline from the book's own run-in headings; `@PdfText[id#section]` tags resolve section-scoped prose. A box that opens in lower case after one that closed without terminal punctuation is joined to it (a column or page turn); `continues` *(additive, 2026-09-17)* is the compiler saying the same of a box that opens with a capital, which the letter test cannot. |
...
| `heading` *(additive, 2026-09-17)* | `box` or `parts: [{box, page?, fixes?, glue?}]`, and one of `number`, `hash` | ok/mismatch, `found`, `title` | `expect` for a name the cookbook does not carry. The runs are joined WITH their fixes; a name the line broke is read in `parts`, a part marked `glue` continuing the word the part before it broke. The check is `number` (the heading's first token folds to exactly this key number, so `36.` never passes on `36U.`) or `hash` (`printKey` of the joined text, `scripts/importer/printed-name.mjs`). On a pass `title` carries the words as printed for the binding to name the document with; on a mismatch it is null and the entry stubs exactly as under `expect`. Nothing v1–v2 changed: an entry that ships its label still uses `expect`. |

**A printed proper name is read, never shipped.** An adventure's own names are
its expression, so an entry compiled from such a row carries a neutral `name`
(`POI 30`, `Organisation 4`) and a `heading` instruction, and the binding names
the document from `fields.name.title`. The locator is a pointer of the same
standing as a page number: the key number the book prints, or a hash of the
name's folded letters that says nothing about what they are.
